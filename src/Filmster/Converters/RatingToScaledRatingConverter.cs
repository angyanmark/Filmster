...
namespace Filmster.Converters
{
    public class RatingToScaledRatingConverter : IValueConverter
    {
        ...
        private double GetRatioFromParameter(object parameter)
        {
            ...
            {
                throw new ArgumentException(string.Format("There must be 2 parameters separated with \"|\". {0} is not a valid parameter.", parameter), nameof(parameter));
            }

            if (!int.TryParse(parameters[0], out int from) || !int.TryParse(parameters[1], out int to))
            {
                throw new ArgumentException(string.Format("Parameters must be int values. {0} or {1} is not an int value.", parameters[0], parameters[1]), nameof(parameter));
            }
